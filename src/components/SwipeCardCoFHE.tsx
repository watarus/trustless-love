...
export const SwipeCardCoFHE: React.FC<Props> = ({
  cofhe,
  contract,
  userAddress,
  targetProfile,
  ...
  const submitVote = async (isLike: boolean) => {
    setVoting(true);
    try {
      console.log("Encrypting vote with CoFHE...");

      // Encrypt the boolean using cofhejs
      const encrypted = await cofhe.encryptBool(isLike);
      console.log("Encrypted vote:", encrypted);

      // CoFHE expects InEbool struct: { ctHash, securityZone, utype, signature }
      const tx = await contract.vote(targetProfile.address, encrypted);

      console.log("Mining transaction...");
      await tx.wait();
      console.log(`Vote casted securely! Like: ${isLike}`);

      // If liked, check if they've also voted (for potential match)
      if (isLike && onMatch) {
        try {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="relative w-80 h-[420px]">
        ...
        >
          <div className="relative w-80 h-[420px] bg-slate-800 rounded-2xl shadow-2xl flex flex-col border-2 border-cyan-500/30 overflow-hidden cursor-grab active:cursor-grabbing select-none">
            {/* LIKE/NOPE Labels */}
            ...
            {/* CoFHE Badge */}
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 px-2 py-0.5 bg-cyan-600/80 rounded text-xs text-white font-mono">
              CoFHE
            </div>
            ...
            <div className="flex-1 flex items-center justify-center bg-gradient-to-b from-slate-700 to-slate-800 p-6">
              <img
                ...
                className="w-44 h-44 rounded-full border-4 border-cyan-500 shadow-lg object-cover"
                draggable={false}
              />
            </div>
            ...
            {voting && (
              <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-cyan-400 text-xs mt-1">via Fhenix CoFHE</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center gap-8">
        ...
        <button
          ...
          disabled={voting}
          className="w-20 h-20 rounded-full bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center hover:from-cyan-400 hover:to-blue-400 hover:scale-110 transition-all shadow-lg shadow-cyan-500/30 disabled:opacity-50 disabled:hover:scale-100"
        >
